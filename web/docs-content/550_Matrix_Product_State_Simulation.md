...
For circuits with larger qubit counts (e.g., $N > 12$), standard statevector simulation becomes memory-prohibitive due to the exponential growth of the state space ($2^N$ complex floats). The `MPSSimulator`, defined in [mps_simulator.py](file:///home/qayum/projects/quantum-virtual-machine/src/qvm/mps_simulator.py), uses a **Tensor Network** representation to simulate low-entanglement quantum circuits up to 20+ qubits.
