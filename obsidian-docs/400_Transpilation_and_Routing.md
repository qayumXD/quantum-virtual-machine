...
Physical quantum processors impose strict connectivity constraints—not all qubits are directly connected. The transpiler module in [transpiler.py](https://github.com/qayumXD/quantum-virtual-machine/blob/main/src/qvm/transpiler.py) maps logical circuits onto a physical topology defined by `TargetArchitecture` in [architecture.py](https://github.com/qayumXD/quantum-virtual-machine/blob/main/src/qvm/architecture.py), inserting `SWAP` gates where necessary.
