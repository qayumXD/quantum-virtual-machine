...
The QVM follows a \textbf{7-layer pipeline architecture} as shown in Figure \ref{fig:architecture_diagram}. Each layer represents a distinct phase in the quantum program lifecycle:

\begin{figure}[h]
\centering
\includegraphics[width=0.9\textwidth]{../uml/QVM_Architecture_Diagram.png}
...
Figure \ref{fig:activity_circuit_execution} shows the complete workflow from loading a circuit file to displaying results. The process includes error handling for invalid files and syntax errors, backend selection (Statevector vs MPS), optional transpilation, and result visualization.

\begin{figure}[h]
\centering
\includegraphics[width=0.75\textwidth]{../uml/Activity_Circuit_Execution.png}
\caption{Activity Diagram - Circuit Execution Flow}
\label{fig:activity_circuit_execution}
\end{figure}
...
Figure \ref{fig:activity_transpilation} details the transpilation workflow, showing how logical circuits are mapped to physical hardware topologies. The process includes gate decomposition, qubit routing (with choice between Greedy and SABRE algorithms), SWAP insertion, and optional mapping restoration.

\begin{figure}[h]
\centering
\includegraphics[width=0.75\textwidth]{../uml/Activity_Transpilation.png}
\caption{Activity Diagram - Transpilation Process}
\label{fig:activity_transpilation}
\end{figure}
...
Figure \ref{fig:activity_simulation} illustrates the simulation workflow, including state initialization, gate application, measurement with collapse, and control flow handling (labels, jumps, conditionals).

\begin{figure}[h]
\centering
\includegraphics[width=0.75\textwidth]{../uml/Activity_Simulation.png}
...
The class diagram in Figure \ref{fig:class_diagram} shows the static structure of the QVM system, including all major classes, their attributes, methods, and relationships.

\begin{figure}[h]
\centering
\includegraphics[width=0.95\textwidth]{../uml/QVM_Class_Diagram.png}
...
Figure \ref{fig:sequence_cli} shows the interaction sequence when a user executes a circuit via the command-line interface.

\begin{figure}[h]
\centering
\includegraphics[width=0.9\textwidth]{../uml/Sequence_CLI_Execution.png}
\caption{Sequence Diagram - CLI Execution}
\label{fig:sequence_cli}
\end{figure}
...
Figure \ref{fig:sequence_api} illustrates the interaction when a client submits a circuit via the REST API.

\begin{figure}[h]
\centering
\includegraphics[width=0.9\textwidth]{../uml/Sequence_API_Request.png}
\caption{Sequence Diagram - API Request}
...
Figure \ref{fig:sequence_transpilation} shows the detailed interaction during the transpilation process.

\begin{figure}[h]
\centering
\includegraphics[width=0.9\textwidth]{../uml/Sequence_Transpilation_Detail.png}
\caption{Sequence Diagram - Transpilation Detail}
\label{fig:sequence_transpilation}
\end{figure}
...
State transition diagrams model the lifecycle of key entities in the system. Figure \ref{fig:state_circuit} shows the state transitions for a quantum circuit as it progresses through the QVM pipeline.

\begin{figure}[h]
\centering
\includegraphics[width=0.85\textwidth]{../uml/State_Circuit_Lifecycle.png}
\caption{State Transition Diagram - Circuit Lifecycle}
\label{fig:state_circuit}
\end{figure}
...
Figure \ref{fig:dfd_level1} shows the high-level data flows between the QVM system and external entities.

\begin{figure}[h]
\centering
\includegraphics[width=0.9\textwidth]{../uml/DFD_Level1.png}
...
Figure \ref{fig:dfd_level2} shows the detailed data flows between internal modules.

\begin{figure}[h]
\centering
\includegraphics[width=0.95\textwidth]{../uml/DFD_Level2.png}
