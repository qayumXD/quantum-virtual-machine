...
\centering
\includegraphics[width=0.95\linewidth,height=0.95\textheight,keepaspectratio]{figures/context_diagram.pdf}
...
\begin{figure}[p]
\centering
\includegraphics[width=0.95\textwidth,height=0.92\textheight,keepaspectratio]{figures/architecture_diagram.pdf}
\caption{System Architecture Diagram — 7-Layer Pipeline}
...
Figure \ref{fig:activity_execution} shows the complete workflow from loading a circuit file to displaying results. The process includes error handling for invalid files and syntax errors, backend selection (Statevector vs MPS), optional transpilation, and result visualization. The execution flow begins with the user providing a circuit file. The system validates the file existence and format, then invokes the appropriate parser. If parsing succeeds, the user can optionally request transpilation for a specific architecture. The system then selects the appropriate simulator based on circuit size and user preference. After simulation, results are displayed and optionally saved to files.

\begin{figure}[p]
\centering
\includegraphics[width=0.85\textwidth,height=0.92\textheight,keepaspectratio]{figures/activity_circuit_execution.pdf}
\caption{Activity Diagram — Circuit Execution Flow}
...
Figure \ref{fig:activity_transpile} details the transpilation workflow, showing how logical circuits are mapped to physical hardware topologies. The process includes gate decomposition, qubit routing (with choice between Greedy and SABRE algorithms), SWAP insertion, and optional mapping restoration. The transpilation process starts by analyzing the logical circuit's connectivity requirements. Complex gates are decomposed into native gates. The routing algorithm then determines the optimal qubit mapping, inserting SWAP gates where necessary to satisfy topology constraints. If mapping restoration is enabled, additional SWAPs are inserted to return qubits to their original logical positions.

\begin{figure}[p]
\centering
\includegraphics[width=0.85\textwidth,height=0.92\textheight,keepaspectratio]{figures/activity_transpilation.pdf}
\caption{Activity Diagram — Transpilation Process}
...
\begin{sidewaysfigure}
\centering
\includegraphics[width=0.95\linewidth,height=0.95\textheight,keepaspectratio]{figures/QVM_Class_Diagram.pdf}
...
Figure \ref{fig:seq_cli} shows the interaction sequence when a user executes a circuit via the command-line interface. The sequence begins with the user invoking the CLI with a circuit file. The CLI loads the file and delegates to the Parser, which creates a QuantumCircuit IR. If transpilation is requested, the CLI invokes the Transpiler with the target architecture. The Transpiler returns a physical circuit with SWAP gates inserted. The CLI then invokes the Simulator, which executes the circuit and returns the statevector and classical memory. Finally, the CLI invokes the Visualizer to display results to the user.

\begin{sidewaysfigure}
\centering
\includegraphics[width=0.95\linewidth,height=0.95\textheight,keepaspectratio]{figures/Sequence_CLI_Execution.pdf}
\caption{Sequence Diagram — CLI Execution Flow}
\label{fig:seq_cli}
\end{sidewaysfigure}
...
\begin{figure}[!htb]
\centering
\includegraphics[width=0.85\textwidth,height=0.75\textheight,keepaspectratio]{figures/state_circuit_lifecycle.pdf}
...
\begin{sidewaysfigure}
\centering
\includegraphics[width=0.95\linewidth,height=0.95\textheight,keepaspectratio]{figures/dfd_level1.pdf}
...
\begin{sidewaysfigure}
\centering
\includegraphics[width=0.95\linewidth,height=0.95\textheight,keepaspectratio]{figures/dfd_level2.pdf}
...
\section{System Interfaces (Current State)}
The Quantum Virtual Machine provides three distinct interfaces to cater to different user workflows: an interactive Web GUI for visual exploration, a Command-Line Interface (CLI) for automation and scripting, and a RESTful API for programmatic integration. The following figures present the current implementation state of these interfaces.

\subsection{Web GUI Interface}
The Web Graphical User Interface provides an intuitive, browser-based environment designed primarily for students and educators. It features a circuit editor, execution controls, and real-time visualization of both the quantum circuit diagram and the resulting measurement probability histogram.

\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth,height=0.45\textheight,keepaspectratio]{figures/ScreenShots/WebUI.png}
\caption{Current State: Web GUI Interface}
\label{fig:current_web_gui}
\end{figure}

\subsection{Command-Line Interface (CLI)}
The Command-Line Interface enables developers and researchers to execute quantum circuits directly from the terminal. It provides robust options for file loading, transpilation routing strategies, and detailed execution logging including statevector and classical memory outputs.

\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth,height=0.45\textheight,keepaspectratio]{figures/ScreenShots/CLI_usage.png}
\caption{Current State: Command-Line Interface Execution}
\label{fig:current_cli}
\end{figure}

\subsection{REST API Interface}
The REST API, built with FastAPI, allows system integrators to interact with the QVM programmatically. It accepts JSON-based circuit definitions via HTTP POST requests and returns comprehensive JSON responses containing execution results and metadata.

\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth,height=0.45\textheight,keepaspectratio]{figures/ScreenShots/API_Usage.png}
\caption{Current State: REST API Interaction}
\label{fig:current_api}
\end{figure}
\FloatBarrier
