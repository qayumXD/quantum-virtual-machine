# Lightweight GUI / API Plan (2026-02-27)

Implement a minimal, cross-platform web UI served by the Python backend. Check items after implementation, tests, and push.

- [x] API skeleton: add `api/app.py` with FastAPI, `/health` and `/run` endpoints; run via `python -m api.app` or `uvicorn api.app:app`.
- [x] Wire backend logic: reuse existing parsers/transpiler/simulator; support JSON or QASM input; expose routing/noise/collapse options.
- [x] CLI flag: add `--api` (or `qvm serve`) to start the API quickly.
- [x] Static client scaffolding: add minimal `web/` (plain HTML/JS) served via FastAPI `StaticFiles`; form for upload/flags; display probabilities/counts.
- [ ] API tests: add pytest client tests hitting `/run` with bell_state (JSON + QASM) asserting probabilities/counts; `/health` returns 200. (partially in place)
- [ ] GUI smoke test: automated or documented manual step to fetch the static index and run a sample request.
- [x] Docs: add `docs/guides/GUI_Usage.md` with start commands, API schema, and client usage.
